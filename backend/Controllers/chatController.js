...
exports.getChat = async (req, res) => {
  ...
  if (req.role === 'pharmacist') {
    const appointments = await Appointment.find({ patient: receiver, pharmacist: process.env.CHAT_PHARMACIST });
    chat = await Chat.findOne({ patient: receiver, pharmacist: process.env.CHAT_PHARMACIST });
    if (chat) {
      res.status(200).json({ chat });
    } else if (appointments) {
      newChat = await Chat.create({ patient: receiver, pharmacist: process.env.CHAT_PHARMACIST });
    ...
    const appointments = await Appointment.find({
      ...
      pharmacist: process.env.CHAT_PHARMACIST,
    });
    chat = await Chat.findOne({ patient: req.user.username, pharmacist: process.env.CHAT_PHARMACIST });
    if (chat) {
      res.status(200).json({ chat });
    } else if (appointments) {
      newChat = await Chat.create({ patient: req.user.username, pharmacist: process.env.CHAT_PHARMACIST });
      res.status(200).json({ newChat });
    } else {
      res.status(200).json({});
    }
